...
// This package also creates its own error structure, so you can type assert if you need to.
...
// ClearOnMenuRun will clear the screen when a menu is run.
...
// The menu will fail if this is not called and multiple selections were selected.
...
// Converts the response string to a slice of int, also validates along the way.
...
func (m *Menu) ynResParse(res string) ([]int, error) {
	...
	re := regexp.MustCompile("^\\s*(?:([Yy])(?:es|ES)?|([Nn])[oO]?)\\s*$")
...
// returns false if it could not find an action for the number options available
